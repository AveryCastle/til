...
public class DelayedRemoteService implements RemoteService {
    ...
    public String call() throws RemoteServiceException {
        ...
        // Since currentTime and serverStartTime are both in nanoseconds, we convert it to seconds by diving by 10e9
        // and ensure floating point division by multiplying it with 1.0 first.
        // We then check if it is greater or less than specified delay and then send the reply.
        double duration = (currentTime - serverStartTime) * 1.0 / (1000 * 1000 * 1000);
        System.out.println(String.format("duration = %f, delay = %d", duration, delay));
        if (duration < delay) {
