...
- 역할
  - 객체의 불변 조건을 유지하기 위한 경계 역할을 한다.
- 구조
  - 루트 + 경계
  - 외부에서 애그리게이트를 다루는 조작은 모두 '루트'를 거쳐야 한다.
- 데메테르의 법칙
  - 객체 지향 프로그래밍에서는 외부에서 내부 객체를 직접 다루는 대신, 내부 객체를 감싸는 객체에 요청하는 형태를 취한다.
  - 객체 간의 메서드 호출에 질서를 부여하기 위한 가이드라인이다.
  - 다음 객체의 메서드만 호출할 수 있음.
    - 객체 자신
    - 인자로 전달받은 객체
    - 인스턴스 변수
    - 해당 컨텍스트에서 직접 생성한 객체
- 애그리게이트의 경계를 정하는 원칙
  - '변경의 단위'
  - 애그리게이트에 대한 변경은 해당 애그리게이트 자신에게만 맡기고, 퍼시스턴시 요청도 애그리게이트 단위로 해야 한다. 따라서, 리포지토리도 애그리게이트마다 하나씩 만들게 된다.
- '식별자'를 이용한 컴포지션 구성
  - 애그리게이트 범위를 벗어나는 객체의 메서드를 호출할 일이 없다.
  - 겍체가 아닌 식별자만 알고 있기 때문에 메모리를 절약할 수 있다.
- 애그리게이트 크기와 조작의 단위
  - 가능한 한 작게 유지하는 것이 좋다.
  - 광범위한 데이터 lock이 걸릴 가능성이 높기때문에 한 트랜잭션에서 여러 애그리게이트를 다루는 것도 가능한 한 피해야 한다.

## 13. 복잡한 조건을 나타내기 위한 '명세'
- 정의
  - 어떤 객체가 그 객체의 평가 기준을 만족하는지 판정하기 위한 객체
- 탄생 배경
  - 객체에 대해 복잡한 평가 절차가 필요한 경우도 있다. 이 평가 절차를 대상 객체 내에 메서드로 두는 것이 부자연스러운 경우가 있다. 도메인 객체에 평가 메서드를 남겨두면 도메인 객체에 대한 의존이 무방비로 증가하는 문제가 발생할 수 있다.
  - 그렇다고 객체 평가 절차는 엄연히 도메인 규칙이기때문에 애플리케이션 서비스에 구현하는 것은 적절하지 않다.
  - 이를 해결해주기 위해 '객체 명세'가 필요하게 되었다.
- 문제점
  - 도메인 규칙이 Repository에 스며드는 현상
    - 명세도 엄연히 도매인 객체이기 때문에 가급적 입출력(Repository)를 안 사용해야 한다는 의견도 있다.
    - `first-class collection`을 사용해서 제네릭 컬렉션 대신 객체 컬렉션을 이용하는 패턴을 사용한다.
    - 이를 통해 중요 규칙을 명세에 정의해서 Repository에서 도메인 규칙이 스며들지 않게 할 수 있다.
  - 명세와 Repository를 함께 사용할 때 `성능` 이슈
    - 주로 Query 를 할 때, 한 번의 `JOIN`으로 해결할 수 있는 기능을 도메인 규칙 분리로 인해 전체 조회한 후에 조건을 걸어서 filtering 함으로써 성능 저하가 발생할 수 있다.
- 성능 해결책
  - CQRS(Command and Query Responsibility Segregation) 으로 완충하기.
    - Command(쓰기 명령)은 주로 도메인에 의한 제약이 있어서 도메인 객체를 적극적으로 활용하고
    - Query(조회 명령)은 프리젠테이션 계층의 성능 요구사항이 더 중요할 수 있기 때문에 최적화를 더 신경써서 애플리케이션을 만든다.

## 14. 아키텍처
- 정의
  - 코드를 구성하는 원칙. 어떤 코드를 어디에 배치할지 기준을 제시해서 고직이 무분별하게 흩어지는 것을 막아준다.
- 2가지
  - 계층형 아키텍처: 프리젠테이션 계층(사용자 인터페이스 계층), 애플리케이션 계층, 도메인 계층, 인프라스트럭처 계층
  - 헥사고날(클린) 아키텍처: 비지니스 관심사를 다루는 도메인을 중심에 두고, 외부 관심사(사용자 인터페이스나 인프러스트럭처)는 바깥에 두어 외부 관심사가 내부 관심사에 의존하게 한다.
    - port-and-adapter 방식으로 외부 관심사는 쉽게 갈아 끼울 수 있게 loose coupling을 이룬다.

## 15. 앞으로의 학습
- 핵심은 도메인으로, 패턴은 도메인을 잘 이해하고 이를 코드로 제대로 표현하기 위한 기술적 보조 수단이다. 패턴을 버릴 줄도 알아야 한다.
- 도메인과 연관된 사람들과 소통하면서 Ubiquitous 언어로 소통하고, 이를 코드에까지 적용해야 한다.
- `Bounded Context의 경계`를 잘 지정해야 한다.
  - 같은 용어이나 그 배경과 목적이 다르다면, 반드시 같은 객체에 넣을 필요가 없다. 패키지를 분리해서 사용할 수 있다.
  - ex) 인증에서의 사용자, 써클에 가입한 사용자
- `컨텍스트맵`을 이용하여 컨텍스트간의 관계를 정의하고 전체 도메인을 바라볼 수 있어야 한다. 
- 테스트작성을 통해 팀간의 변경 사항에 대한 신뢰를 쌓는다. 가장 좋은 건 하위 컴포넌트 팀이 상위 컴포넌트 팀을 의식하지 않고 자유롭게 변경할 수 있는 것이다.
- `에릭 에반스의 '도메인 주도 설계'`를 꼭 읽어봐야 한다.